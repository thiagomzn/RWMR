...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
        <nav class="sb-topnav navbar navbar-dark bg-dark">
            <h1 class="navbar-brand ps-3">RWMR - Inscribirse a mesas</h1>
        </nav>
        <div id="layoutSidenav">
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Mesas creadas</h1>
                        <br>
                        <br>
                        <?php
$conexion = mysqli_connect("localhost", "root", "", "rwmr");
$result = mysqli_query($conexion, "SELECT * FROM asignatura WHERE `ID_año/división` < 3 AND `Tipo` = 'Áulica'");
...
if ($result->num_rows > 0) {
    ...
    echo "<table = tablep-c>";
    echo "<thead><tr><th>Año/división</th><th>Nombre</th><th>Fecha</th><th>profesores</th><th>Inscribirse</th></tr></thead>";
    while($fila = $result->fetch_assoc()) {
        echo "<tbody></td><td>".$fila["ID_año/división"]."</td><td>".$fila["Nombre"]."</td></tr></tbody>";
    }
} else {
...
$conexion = mysqli_connect("localhost", "root", "", "rwmr");
$result = mysqli_query($conexion, "SELECT * FROM examen");
if ($conexion->connect_error) {
    die("Error de conexión: " . $conexion->connect_error);
}
...
if ($result->num_rows > 0) {
    while($fila = $result->fetch_assoc()) {
        echo "<tbody></td><td>".$fila["ID_año/división"]."</td><td>".$fila["Nombre"]."</td><td><form>
...
?>
                        <table class="table-RWMR">
                            <thead>
                                <tr>
                                    <th class="año/div">Año/división</th>
                                    <th class="nombre">Nombre</th>
                                    <th class="fecha">Fecha</th>
                                    <th class="profesores">Profesores</th>
                                    <th class="incribirse">Inscribirse</th>
                                </tr>
                                <tr>
                                    <td>6to B </td>
